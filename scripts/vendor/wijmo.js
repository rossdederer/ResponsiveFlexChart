...
(function (wijmo) {
    ...
    */
    var Globalize = (function () {
        ...
        */
        Globalize.formatNumber = function (value, format, trim) {
        ...
        */
        Globalize.parseFloat = function (value, format) {
            var dp = wijmo.culture.Globalize.numberFormat['.'], rx = new RegExp('[^\\d\\' + dp + ']', 'g'), neg = value.indexOf('-') > -1 || (value.indexOf('(') > -1 && value.indexOf(')') > -1) ? -1 : +1, pct = value.indexOf('%') > -1 ? .01 : 1, m = format ? format.match(/,+/) : null, scale = m ? m[0].length * 3 : 0;
        ...
        */
        Globalize.parseDate = function (value, format) {
            ...
            var cal = wijmo.culture.Globalize.calendar, cjk = 'a-z\u00C0-\u017F\u3000-\u30ff\u4e00-\u9faf', rxv = new RegExp('(\\' + cal['/'] + ')|' + '(\\' + cal[':'] + ')|' + '(\\d+)|' + '([' + cjk + ']+)|' + '([' + cjk + '\\.]{2, })', 'gi'), vparts = value.match(rxv), fparts = Globalize._parseDateFormat(format), offset = 0, year = -1, month = 0, day = 1, hour = 0, min = 0, tzm = 0, sec = 0, ms = 0, era = -1;
            ...
            for (var i = 0; i < fparts.length && vparts; i++) {
                ...
                switch (fparts[i]) {
                    ...
                    case 'tt':
                    case 't':
                        if (cal.pm[0] && pval.toUpperCase() == cal.pm[0] && hour < 12) {
                            hour += 12;
                        }
                        break;
                    case 'yyyy':
        ...
        */
        ...
        Globalize._parseDateFormat = function (format) {
            ...
            for (start = 0; start > -1 && start < format.length; start++) {
                ...
                if (c == '\'' || c == '"') {
                    ...
                    if (end > -1) {
                        parts.push(format.substring(start + 1, end));
                    }
                    start = end;
                } else {
                    end = start + 1;
        ...
        Globalize._formatDatePart = function (d, format, part) {
            var cal = wijmo.culture.Globalize.calendar, era = 0, year = 0, ff = 0, plen = part.length;
            switch (part) {
                case 'dddd':
    ...
    var Control = (function () {
        ...
        */
        function Control(element, options, invalidateOnResize) {
            ...
            */
            ...
            var self = this;
            self.addEventListener(host, 'focus', function () {
                setTimeout(function () {
                    self._updateFocusState();
                }, 10);
            }, true);
            self.addEventListener(host, 'blur', function () {
                setTimeout(function () {
                    self._updateFocusState();
                }, 100);
            }, true);
            ...
            if (Control._touching == null) {
                ...
                if ('ontouchstart' in window || 'onpointerdown' in window) {
                    ...
                    if ('ontouchstart' in window) {
                        this.addEventListener(b, 'touchstart', ts);
                        this.addEventListener(b, 'touchend', te);
                        this.addEventListener(b, 'touchcancel', te);
                        this.addEventListener(b, 'touchleave', te);
                    } else if ('onpointerdown' in window) {
                        this.addEventListener(b, 'pointerdown', ts);
                        this.addEventListener(b, 'pointerup', te);
                        this.addEventListener(b, 'pointerout', te);
                        this.addEventListener(b, 'pointercancel', te);
                        this.addEventListener(b, 'pointerleave', te);
                    }
                }
            }
        ...
        */
        Control.prototype.invalidate = function (fullUpdate) {
            ...
            var self = this;
            self._fullUpdate = self._fullUpdate || fullUpdate;
            if (self._toInv) {
                clearTimeout(self._toInv);
                self._toInv = null;
            }
            if (!self.isUpdating) {
                self._toInv = setTimeout(function () {
                    self.refresh(self._fullUpdate);
        ...
        */
        ...
        Control.prototype._updateFocusState = function () {
            ...
            if (focus != this._focus) {
                ...
                wijmo.toggleClass(this._e, 'wj-state-focused', focus);
            }
            ...
            CollectionView.prototype._compareItems = function () {
                ...
                return function (a, b) {
                    for (var i = 0; i < sortDsc.length; i++) {
                        ...
                        // NOTE: not using 'isString' to improve performance
                        if (typeof (v1) === 'string')
            ...
            CollectionView.prototype.remove = function (item) {
                ...
                if (index > -1) {
                    ...
                    if (this._trackChanges == true) {
                        ...
                        // add to removed list unless it was pending
                        if (this._chgRemoved.indexOf(item) < 0 && !pendingNew) {
...
(function (wijmo) {
    ...
    */
    function showPopup(popup, ref, above, fadeIn) {
        if (typeof ref === "undefined") { ref = null; }
        if (typeof above === "undefined") { above = false; }
        if (typeof fadeIn === "undefined") { fadeIn = false; }
        // assume the popup parent will be the document body
        ...
        if (ref instanceof HTMLElement) {
            ...
            // 2 - adjust the parent to account for ancestors with position:fixed/relative
            var prel;
        ...
        if (ref instanceof MouseEvent) {
            if (ref.clientX <= 0 && ref.clientY <= 0 && ref.target) {
                ...
                pos.x = Math.max(0, ref.clientX - sz.width / 2);
                pos.y = Math.max(0, ref.clientY - sz.height / 2);
            }
            ...
            pos.y = Math.max(0, (innerHeight - sz.height) / 2);
        } else {
    ...
    var _MaskProvider = (function () {
        ...
        */
        ...
        _MaskProvider.prototype._hKeyPress = function (e) {
            if (e.charCode && this._mskArr.length) {
                ...
                if (start < this._firstPos) {
                    ...
                    wijmo.setSelectionRange(el, start, start);
                }
        ...
        _MaskProvider.prototype._validatePosition = function (start) {
            ...
            wijmo.setSelectionRange(this.input, start, start);
        };
